...
#let invoice(
  ...
  if data != none {
    ...
    invoice-date = parse-date(data.at("invoice-date", default: invoice-date))
  ...
  [
    ...
    #grid(
      ...
      grid.hline(stroke: (thickness: 0.5pt)),
      [*Pos.*], [*Beschreibung*], [*Preis*], grid.hline(),
      ..items, grid.hline(),
      [], [
        #set align(end)
        Summe:
      ], [#format_currency(if kleinunternehmer {total} else {(1.0 - vat) * total})€], grid.hline(start: 2),
      ...
      [
        #set align(end)
        *Gesamt:*
      ], [*#format_currency(total)€*], grid.hline(start: 2),
    )
  ]
